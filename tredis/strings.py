...
class StringsMixin(object):
    ...
    def setbit(self, key, offset, bit):
        ...
        created. The string is grown to make sure it can hold a bit at offset.
        The offset argument is required to be greater than or equal to ``0``,
        and smaller than 2 :sup:`32` (this limits bitmaps to 512MB). When the
        ...
        .. warning:: When setting the last possible bit (offset equal to
           ...
           time. On a 2010 MacBook Pro, setting bit number 2:sup:`32`-1 (512MB
           allocation) takes ~300ms, setting bit number 2:sup:`30`-1 (128MB
           allocation) takes ~80ms, setting bit number 228 -1 (32MB allocation)
           takes ~30ms and setting bit number 2:sup:`26`-1 (8MB allocation)
           takes ~8ms. Note that once this first allocation is done, subsequent
           calls to :meth:`~tredis.RedisClient.setbit` for the same key will
           not have the allocation overhead.
